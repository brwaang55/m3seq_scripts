...
def trim_R2(sample, ID, name, bc_to_seq):
    ...
    os.system(
        'cutadapt -a "' + sequence + 'TCTGGCGTAGGAGG;min_overlap=4" -a "TCTGGCGTAGGAGG;min_overlap=8" --minimum-length 16 -o ' + sample + '_R2_trimmed_v3_UMI_processed/' + R2_file_name + '_R2_trimmed.fastq.gz ' + sample + '_selected_cells/' + R2_file_name + '.fastq.gz >> ' + sample + '_R2_trim_v3.log')
    print(R2_file_name)
...
bc1_list = open('/Genomics/grid/users/bw21/gitrepos/scBactoSeq/data/Barcodes/')
bc_to_seq = {}
...
os.system('mkdir ' + sample + '_R2_trimmed_v3_UMI_processed')
for line in table:
    name = line.split('\t')[1]
    trim_R2(sample, ID, name, bc_to_seq)
# os.system('rm -r ' + sample + '_UMI_processed')
